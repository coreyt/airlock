...
PROVIDER_SCHEMAS: dict[str, ProviderSchema] = {
    "anthropic": ProviderSchema(fields=[
    ...
    "openai": ProviderSchema(fields=[
        _temperature(1.0, 2.0),
        _max_tokens(1024, 128_000),
        _top_p(),
        ParamField("frequency_penalty", "Frequency Penalty", "float", 0.0, -2.0, 2.0),
        ParamField("presence_penalty", "Presence Penalty", "float", 0.0, -2.0, 2.0),
        _stop(),
        _system(),
    ]),
    "gemini": ProviderSchema(fields=[
        _temperature(1.0, 2.0),
        ParamField("max_output_tokens", "Max Output Tokens", "int", 1024, 1, 65_536),
        _top_p(),
        _top_k(),
        _stop(),
        _system(),
    ]),
    "mistral": ProviderSchema(fields=[
    ...
    "perplexity": ProviderSchema(fields=[
        _temperature(0.2, 1.5),
        _max_tokens(1024, 128_000),
        _top_p(),
        ParamField("return_citations", "Return Citations", "bool", False),
        ParamField(
            "search_recency_filter", "Search Recency", "enum", None,
            choices=["month", "week", "day", "hour"],
        ),
        _system(),
    ]),
    "tavily": ProviderSchema(fields=[
        ParamField("max_results", "Max Results", "int", 5, 1, 20),
        ParamField(
            "search_depth", "Search Depth", "enum", "basic",
            choices=["basic", "advanced"],
        ),
    ]),
    "local": ProviderSchema(fields=[
        _temperature(0.7, 2.0),
        _max_tokens(1024, 32_768),
        _top_p(),
        _top_k(),
        ParamField("repetition_penalty", "Repetition Penalty", "float", None, 0.0, 2.0),
        _stop(),
    ]),
}

# Fallback for unknown providers or the "all" meta-provider.
DEFAULT_SCHEMA = ProviderSchema(fields=[
    _temperature(0.7, 2.0),
    _max_tokens(1024),
    _top_p(),
    _top_k(),
