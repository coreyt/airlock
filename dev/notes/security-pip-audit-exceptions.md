...
| 2026-08-07 | 0.5.11 (internal) | **Still required.** With the three flags, `pip-audit 2.7.3` reports clean over the 0.5.11 lock (which bumps `fathomdb` to 0.8.21 — no new findings). Both upstream pins unchanged: `litellm[proxy]` → `cryptography>=48.0.1,<49.0`; `presidio-anonymizer` → `cryptography>=48.0.1,<49.0.0`; installed `cryptography 48.0.1`. Still no direct `cryptography` import in `airlock/`. Suppressions retained. |
